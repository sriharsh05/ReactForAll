import { previewQuestion, fieldAnswer, formField } from "../types/formTypes";
        ...
        case "prev": {
          return (
            state && {
              ...
              currentQuestion: action.getIndexQuestionCB(
        ...
        if (answer.form_field === action.id) {
          ...
          if (answer.value !== action.value) {
            newState = state.map((answer) => ({
              ...answer,
              ans:
                answer.form_field === action.id ? action.value : answer.value,
            }));
          }
        }
      });
